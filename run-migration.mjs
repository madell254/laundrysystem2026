import { createRequire as __cr } from 'node:module'; import __p from 'node:path'; import __u from 'node:url'; globalThis.require=__cr(import.meta.url); globalThis.__filename=__u.fileURLToPath(import.meta.url); globalThis.__dirname=__p.dirname(globalThis.__filename);
...
if (!DATABASE_URL) {
  console.error("\nERROR: DATABASE_URL not set. Please check your .env file.\n");
  process.exit(1);
}
var sqlPath = join(__dirname, "migration.sql");
if (!existsSync(sqlPath)) {
  console.error("\nERROR: migration.sql not found.\n");
  process.exit(1);
}
var sql = readFileSync(sqlPath, "utf-8");
var client = new Client({ connectionString: DATABASE_URL });
var NEW_DEPARTMENTS = [
  "PMO Department",
...
async function run() {
  try {
    ...
    for (const name of NEW_DEPARTMENTS) {
      await client.query("INSERT INTO departments (name) VALUES ($1) ON CONFLICT DO NOTHING", [name]);
    }
    console.log(`  Inserted ${NEW_DEPARTMENTS.length} departments.`);
